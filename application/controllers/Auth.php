...
class Auth extends CI_Controller {
	...
	function login()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			else
			{
                ...
                echo json_encode(array('vresult'=>'error','message'=>$this->ion_auth->errors(),'csrf_hash'=>$csrf_hash));
            }
		}else{
	...
	function logout()
	{
        ...
        $this->session->set_userdata('demo',0);
	...
	{  
		...
		if ($this->form_validation->run() == false)
		{
            ...
            $csrf_hash = $this->security->get_csrf_hash();
            echo json_encode(array('vresult'=>'error','message'=>validation_errors(),'csrf_hash'=>$csrf_hash));
		...
		else
		{
            ...
            if(!isset($skip)) {
                ...
                if ($forgotten) {
                    ...
                    $csrf_hash = $this->security->get_csrf_hash();
                    echo json_encode(array('vresult' => 'success','csrf_hash' => $csrf_hash));
                } else {
                    //$this->session->set_flashdata('message', $this->ion_auth->errors());
                    //redirect("auth/forgot_password", 'refresh');
                    $csrf_hash = $this->security->get_csrf_hash();
                    echo json_encode(array('vresult' => 'error', 'csrf_hash' => $csrf_hash));
	...
	public function reset_password($code = NULL)
	{
        ...
        if ($user)
		{
			...
			else
			{
				...
				if ($user->id != $this->encryption->decrypt($this->input->post('user_id')))
				{
                    ...
                    $csrf_hash = $this->security->get_csrf_hash();
                    echo json_encode(array('vresult'=>'error','message'=>'Something went wrong. Try again.','csrf_hash'=>$csrf_hash));
		...
		else
		{
            ...
            if (!$this->input->is_ajax_request()) {
                $this->session->set_flashdata('message', '<strong>Oops</strong> ... You have already changed your password using this request. Click <a data-toggle="modal" data-target="#forgot-modal" href="#">here</a> to receive a new password reset request.');
                redirect("/", 'refresh');
            }else {
                $csrf_hash = $this->security->get_csrf_hash();
                echo json_encode(
                    array('vresult' => 'error', 'message' => '<strong>Oops</strong> ... You have already changed your password using this request. Click <a data-toggle="modal" data-target="#forgot-modal" href="#">here</a> to receive a new password reset request.', 'csrf_hash' => $csrf_hash)
                );
            }
		}
	}
	...
	/**
	 * Check if we have all the questions answered
	 */
	protected function _setup_check($frm_data='') {
		...
		// get a list of all questions to check against
		$questions = $this->Application_model->get(array('table'=>'questionnaire'));
		foreach($questions as $question){
			$check_list[] = $question['id'];
		}

		$frm_data_arr = json_decode($frm_data);

		foreach($frm_data_arr as $data) {

			if (in_array($data->name, $check_list) && trim($data->value)!='') {
				if (false !== $key = array_search($data->name, $check_list)) {
	...
	function create_user()
	{
		...
		if($return_q != ''){
			...
			if ($this->form_validation->run() == true) {
				$username = strtolower($this->input->post('first_name')) . ' ' . strtolower(
